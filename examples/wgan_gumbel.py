...
try:
    ...
    G = Seq2seq(encoder, decoder, decode_function=helper.gumbel_softmax)
...
for epoch in range(100):
    ...
    for step, batch in enumerate(train_iter):
        ...
        decoder_outputs, decoder_hidden, other = G.forward(
            src_seq, src_length.tolist(), target_variable=None, tau=1, hard=True)
        # pred = torch.cat([x for x in other['sequence']], dim=1)
        # loss_G = D(pred).sum() * 100000
        # print(decoder_outputs)

        optim_G.zero_grad()
        loss_G = D(decoder_outputs, input_onehot=True).sum() * 10000

        print(loss_G)
        loss_G.backward()
        print([(k, v) for k, v in G.named_parameters()])
        optim_G.step()
        print([(k, v) for k, v in G.named_parameters()])
