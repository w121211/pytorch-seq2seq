...
#             pool.append_real(src_seq)
#
#             if len(pool.fakes) > 200:
#                 break
#
...
for epoch in range(100):
    ...
    for step, batch in enumerate(train_iter):
        ...
        loss_G = 0.5 * torch.mean((log(D_fake) - log(1 - D_fake)) ** 2)
        loss_G.backward()
        optim_G.step()
        ...
        if step % 100 == 0:
            ...
            print('[step %d] loss_D %.4f, D_fake %.4f' % (
                epoch * len(train_iter) + step, loss_G.data[0], torch.mean(D_fake).data[0]))
