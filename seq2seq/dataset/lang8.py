...
class Lang8(TranslationDataset):
    ...
    @classmethod
    def preprocess(cls, to_dir='../../data/lang8'):
        '''建立lang-8的train & test datasets
        '''
        train_err_path = os.path.join(to_dir, 'train.err')
        train_cor_path = os.path.join(to_dir, 'train.cor')
        test_err_path = os.path.join(to_dir, 'test.err')
        test_cor_path = os.path.join(to_dir, 'test.cor')

        tokenizer = MosesTokenizer()

        with open(cls.path) as f, \
                open(train_err_path, 'w+') as train_err, \
                open(train_cor_path, 'w+') as train_cor, \
                open(test_err_path, 'w+') as test_err, \
                open(test_cor_path, 'w+') as test_cor:
            count = 0
            for line in f:
                try:
                    j = json.loads(line)
                ...
                for i in range(len(text)):
                    if not text[i].endswith(('.', '?', '!')):  # 簡單測試是否為句子
                        continue

                    for corrected in j[5][i]:  # 有數個更改者，不一定每個都有更改
                        if corrected is None:
                            continue

                        # remove tags [f-blue], [f-red], [f-bold]
                        for x in ('[f-blue]', '[f-red]', '[f-bold]'):
                            corrected = corrected.replace(x, ' ').replace(x.replace('[', '[/'), ' ')

                        # remove slashes [sline] ... [/sline]
                        corrected = re.sub(r'\[sline\].*\[/sline\]', ' ', corrected)

                        # 清理target
                        if fuzz.ratio(text[i], corrected) < 80:  # similarity
                            corrected = re.sub(r'\(.*\)', ' ', corrected)  # 移除括號
                            corrected = re.sub(r'\[.*\]', ' ', corrected)  # 移除中括號
                            # sim = similarity(src[i], trg, False)
                            if fuzz.ratio(text[i], corrected) < 60:
                                continue

                        # 輸出 pairs
                        # print(' '.join(tokenizer.tokenize(src[i])) + '\n')
                        if count % 10 == 0:
                            test_err.write(
                                ' '.join(tokenizer.tokenize(text[i], escape=False)) + '\n')
                            test_cor.write(
                                ' '.join(tokenizer.tokenize(corrected, escape=False)) + '\n')
                        else:
                            train_err.write(
                                ' '.join(tokenizer.tokenize(text[i], escape=False)) + '\n')
                            train_cor.write(
